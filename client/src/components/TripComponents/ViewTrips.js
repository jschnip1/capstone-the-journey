...
function ViewTrips({ trip, owner }) {
  ...
  return (
    <>
      <Item.Group>
        <h3>
          Start Date: {trip.startTime}{" "}
          <span id="end-date-trip">End Date: {trip.endTime}</span>
        </h3>
        {owner ? (
          <Rating
            icon="star"
            rating={trip.tripReview}
            maxRating={5}
            onRate={handleRate}
          />
        ) : (
        ...
        {trip.locations.map((a) => (
          <Trip key={a.location.sortOrder} trip={a.location} />
        ))}
      </Item.Group>
    </>
  );
