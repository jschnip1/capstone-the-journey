...
public class AppUserService implements UserDetailsService {
    ...
    private void validate(String username) {
        ...
        if (checkForDuplication(username)) {
            throw new ValidationException("username is already in use");
        }
    }

    private boolean checkForDuplication(String username){
        AppUser duplicate = repository.findByUsername(username);
        if(duplicate == null || !duplicate.isEnabled()) {
            return false;
        }
        return true;
    }
